...
SierpinskiGasket::SierpinskiGasket(){
	...
	// generate points to be used by retained_mode_graphic
...
void SierpinskiGasket::draw() {
	...
	// the initial implmentation.
	//this->initial_draw();

	// the immediate mode
	//this->immediate_mode_graphic();

	// the retained mode graphics
	this->retained_mode_graphic();
}

void SierpinskiGasket::immediate_mode_graphic()
{
	this->draw_initial_triangle();
	
	glColor3f(1.0f, 0.0f, 0.0f);
	glBegin(GL_POINTS);
	Point p;
	for (int i = 0; i <= 10000; i++) {
		p = this->get_a_new_point();
		glVertex2f(p.x, p.y);
	}
	glEnd();
...
void SierpinskiGasket::retained_mode_graphic()
{
	this->draw_initial_triangle();
...
void SierpinskiGasket::initial_draw()
{
	this->draw_initial_triangle();
	...
	glEnd();
	// add then draw next frame a new point
	this->list_of_points.push_back(this->get_a_new_point());
	
}

void SierpinskiGasket::draw_initial_triangle()
{
